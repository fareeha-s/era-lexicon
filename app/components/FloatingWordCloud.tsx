...
const GifImageThumbnail = memo(function GifImageThumbnail({ item }: { item: any }) {
  return (
    <div
      className="relative w-20 h-20 md:w-24 md:h-24 rounded-lg overflow-hidden border border-white/20 hover:border-white/40 transition-all hover:scale-105 flex-shrink-0 will-change-transform"
      title={item.title}
    >
      <img
        src={item.url}
...
const VideoThumbnail = memo(function VideoThumbnail({ item, onSelect }: { item: any, onSelect?: (id: string) => void }) {
  ...
  return (
    <button
      className="group relative w-20 h-14 md:w-28 md:h-20 rounded-lg overflow-hidden border border-white/20 hover:border-white/40 transition-all hover:scale-105 flex-shrink-0 will-change-transform"
...
const SongCard = memo(function SongCard({ song }: { song: Song }) {
  return (
    <a
      ...
      className="group block w-32 md:w-40 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/30 rounded-lg p-2 md:p-3 transition-all hover:scale-105 flex-shrink-0 will-change-transform"
...
const TweetCard = memo(function TweetCard({ tweet }: { tweet: Tweet }) {
  return (
    <div className="block w-40 md:w-48 bg-white/5 border border-white/10 rounded-lg p-2 md:p-3 flex-shrink-0">
      ...
      {shuffledContent.map((item, index) => {
        if (item.type === 'word') {
          ...
          return (
            <span
              key={item.id}
              className="floating-word font-light tracking-wide text-white/50 hover:text-white transition-colors cursor-pointer select-none"
              style={{
